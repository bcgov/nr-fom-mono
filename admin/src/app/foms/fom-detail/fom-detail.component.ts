...
export class FomDetailComponent implements OnInit, OnDestroy {
  ...
  private workflowStateChangeRequest: ProjectWorkflowStateChangeRequest = <
    ProjectWorkflowStateChangeRequest
  >{};
  ...
  ngOnInit() {
    ...
    if (this.project.id) {
      // subscribe only when first project init successfully.
      this.projectUpdateTriggered$
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe(() => {
          this.projectService
            .projectControllerFindOne(this.project.id)
            .subscribe((data) => {
  ...
  public deleteAttachment(id: number) {
    const dialogRef = this.modalSvc.openConfirmationDialog(
      `You are about to delete this attachment. Are you sure?`,
      "Delete Attachment"
    );
  ...
  onSuccess() {
    this.router.navigate([`a/${this.project.id}`]).then(() => {
  ...
  deleteFOM() {
    const dialogRef = this.modalSvc.openConfirmationDialog(
      `You are about to withdraw FOM ${this.project.id} - ${this.project.name}. Are you sure?`,
      "Withdraw FOM"
    );
    dialogRef.afterClosed().subscribe((confirm) => {
      if (confirm) {
  ...
  finalizeFOM() {
    const dialogRef = this.modalSvc.openConfirmationDialog(
      `You are about to finalize FOM ${this.project.id} - ${this.project.name}. Are you sure?`,
      "Finalize FOM"
    );
    dialogRef.afterClosed().subscribe((confirm) => {
      if (confirm) {
        ...
        this.projectService
          .projectControllerStateChange(this.project.id, {
  ...
  public async publishFOM() {
    ...
    if (ready) {
      this.workflowStateChangeRequest.workflowStateCode =
        WorkflowStateEnum.Published;
      this.workflowStateChangeRequest.revisionCount =
        this.project.revisionCount;

      this.isPublishing = true;
      try {
        await this.projectService
          .projectControllerStateChange(
            this.project.id,
            this.workflowStateChangeRequest
          )
          .toPromise();
      } finally {
  ...
  public async setCommentClassification() {
    ...
    try {
      await this.projectService
        .projectControllerCommentClassificationMandatoryChange(
          this.project.id,
          {
            commentClassificationMandatory:
              !this.project.commentClassificationMandatory,
            revisionCount: this.project.revisionCount,
          }
        )
        .toPromise();
  ...
  */
  ...
  public isDeleteAttachmentAllowed(attachment: AttachmentResponse) {
    return this.attachmentResolverSvc.isDeleteAttachmentAllowed(
      attachment.attachmentType.code,
      this.project.workflowState.code
    );
  }
  ...
  public openChangeEndDateModal() {
    // open modal
    this.changeEndDateModal = this.ngbModalService.open(
    ...
    let modalInstance = this.changeEndDateModal
      .componentInstance as EnddateChangeModalComponent;
    modalInstance.projectId = this.project.id;
    modalInstance.currentCommentingClosedDate =
      this.project.commentingClosedDate;
  ...
  private calculateDaysRemaining() {
    this.daysRemaining = moment(this.project.commentingClosedDate).diff(
      moment(this.today),
      "days"
    );
  ...
  private validatePublishReady() {
    let ready = true;
    if (
      moment(this.project.commentingClosedDate).diff(
        moment(this.project.commentingOpenDate),
        "days"
      ) < 30
    ) {
      ready = false;
      this.modalSvc.openWarningDialog(
        'Comment End Date must be at least 30 days after Comment Start Date when "Publish" is pushed.'
      );
    }
    ...
    if (!this.spatialDetail || this.spatialDetail.length == 0) {
      ready = false;
      this.modalSvc.openWarningDialog(
        'Proposed FOM spatial file should be uploaded before "Publish" is pushed.'
      );
    }

    if (
      moment(this.project.commentingOpenDate).diff(moment(this.today), "days") <
      1
    ) {
      ready = false;
      this.modalSvc.openWarningDialog(
        'Comment Start Date must be at least one day after "Publish" is pushed.'
      );
    }
    return ready;
  }
