...
import {
  CommentScopeOpt,
  COMMENT_SCOPE_CODE,
} from "@admin-core/utils/constants/constantUtils";
...
export class ReviewCommentsComponent implements OnInit, OnDestroy {
  ...
  public commentScopeCodes = _.keyBy(
    this.stateSvc.getCodeTable("commentScopeCode"),
    "code"
  );
  ...
  ngOnInit() {
    if (
      this.commentListScrollContainer &&
      this.commentListScrollContainer.nativeElement
    ) {
    ...
    this.projectSvc
      .projectControllerFindOne(this.projectId)
      .toPromise()
      .then((result) => {
        this.project = result;
      });

    this.spatialFeatureService
      .spatialFeatureControllerGetForProject(this.projectId)
      .toPromise()
      .then((spatialDetails) => {
        this.commentScopeOpts =
          CommonUtil.buildCommentScopeOptions(spatialDetails);
        this.selectedScope = this.commentScopeOpts.filter(
          (opt) => opt.commentScopeCode == null
        )[0]; // allOpt;
      });
  ...
  getProjectComments() {
    const filterData = (scope: CommentScopeOpt) =>
      map((comments: PublicCommentAdminResponse[]) => {
        ...
        fPublicComments = comments.filter((comment) => {
          ...
          return (
            comment.commentScope.code === scope.commentScopeCode &&
            ((comment.scopeCutBlockId &&
              comment.scopeCutBlockId == scope.scopeId) ||
              (comment.scopeRoadSectionId &&
                comment.scopeRoadSectionId == scope.scopeId))
          );
        });
    ...
    return this.commentSvc
      .publicCommentControllerFind(this.projectId)
  ...
  canReplyComment() {
    const userCanModify = this.user.isAuthorizedForClientId(
      this.project.forestClient.id
    );
    return (
      userCanModify &&
      (this.project.workflowState["code"] === "COMMENT_OPEN" ||
        this.project.workflowState["code"] === "COMMENT_CLOSED")
    );
  }

  async saveComment(
    update: PublicCommentAdminUpdateRequest,
    selectedComment: PublicCommentAdminResponse
  ) {
    ...
    try {
      this.loading = true;
      const result = await this.commentSvc
        .publicCommentControllerUpdate(id, update)
        .toPromise();
