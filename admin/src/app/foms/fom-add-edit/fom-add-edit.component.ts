...
export class FomAddEditComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public cancelChanges() {
    ...
    const routerFragment = this.isCreate
      ? ["/search"]
      : ["/a", this.originalProjectResponse.id];
    ...
    this.route.url
      .pipe(
        ...
        switchMap((url) => {
          ...
          return this.isCreate
            ? of({})
            : this.projectSvc.projectControllerFindOne(
      ...
      .subscribe((data: ProjectResponse) => {
        if (!this.isCreate) {
          ...
          this.forestClientSelect =
            this.originalProjectResponse.forestClient.id;

          this.isPublishState =
            this.originalProjectResponse.workflowState.code ===
            WorkflowStateEnum.Published;

          this.attachmentResolverSvc
            .getAttachments(this.originalProjectResponse.id)
            .then((result) => {
              for (const attachmentResponse of result) {
                if (
                  attachmentResponse.attachmentType.code ===
                  AttachmentTypeEnum.PUBLIC_NOTICE
                )
                  this.attachmentsInitialNotice.push(attachmentResponse);
                else this.attachments.push(attachmentResponse);
              }
            })
            .catch((error) => {
              console.error(error);
  ...
  submit() {
    ...
    projectCreate.commentingOpenDate = cmoDateVal
      ? moment(cmoDateVal).format("YYYY-MM-DD")
      : null;
    projectCreate.commentingClosedDate = cmcDateVal
      ? moment(cmcDateVal).format("YYYY-MM-DD")
      : null;
    this.projectSvc
      .projectControllerCreate(projectCreate)
      .toPromise()
  ...
  async saveApplication() {
    ...
    this.validate();
    const { id, forestClient, workflowState, ...rest } =
      this.originalProjectResponse;
    ...
    try {
      const cmoDateVal = this.fg.get("commentingOpenDate").value;
      const cmcDateVal = this.fg.get("commentingClosedDate").value;
      projectUpdateRequest.commentingOpenDate = cmoDateVal
        ? moment(cmoDateVal).format("YYYY-MM-DD")
        : null;
      projectUpdateRequest.commentingClosedDate = cmcDateVal
        ? moment(cmcDateVal).format("YYYY-MM-DD")
        : null;
      await this.projectSvc
        .projectControllerUpdate(id, projectUpdateRequest)
        .toPromise();
      ...
      if (this.supportingDocuments.length > 0) {
        ...
        fileContent = new Blob([this.supportingDocContent], {
          type: file.type,
        });
        await this.attachmentUploadSvc
          .attachmentCreate(
            ...
            fileContent,
            id,
            AttachmentTypeEnum.SUPPORTING_DOC
          )
          .pipe(tap((obs) => console.log(obs)))
          .toPromise();
      }
  ...
  validateClosedDate(closedDate: Date): void {
    ...
    const defaultClosedDate = moment(commentingOpenDateField.value).add(
      30,
      "d"
    );
    const diff = moment(closedDate.toISOString()).diff(
      defaultClosedDate,
      "days"
    );
    if (diff < 0) {
      ...
      if (
        ...
        this.modalSvc.openWarningDialog(
          `Commenting Closed Date cannot be before ${defaultClosedDate.format(
            "YYYY-MM-DD"
          )}`
        );
      }
      ...
      if (!this.isCreate) {
        const closeDatePipe = this.datePipe.transform(
          this.originalProjectResponse.commentingClosedDate,
          "yyyy-MM-dd"
        );
  ...
  toggleClosedDate(newCommentingOpenDate: Date): void {
    ...
    if (newCommentingOpenDate) {
      ...
      this.validateClosedDate(
        commentingClosedDateField.value
          ? moment(commentingClosedDateField.value).toDate()
          : null
      );
  ...
  onSuccessAttachment(id: number) {
    this.router.navigate([`a/${id}/edit`]).then(() => {
  ...
  private initializeFormFields(
    ...
    const openDatePipe = this.datePipe.transform(
      fg.value.commentingOpenDate,
      "yyyy-MM-dd"
    );
    ...
    const closeDatePipe = this.datePipe.transform(
      fg.value.commentingClosedDate,
      "yyyy-MM-dd"
    );
    commentingClosedDateField.setValue(closeDatePipe);
    if (
      (user.isMinistry && !user.isForestClient) ||
