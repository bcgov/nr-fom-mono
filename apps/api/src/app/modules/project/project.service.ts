...
export class ProjectService extends DataService<Project, Repository<Project>> {
  ...
  async findPublicSummaries():Promise<ProjectPublicSummaryDto[]> {

    const forestClientName = 'ER';
    ...
    try {
      const query = this.repository.createQueryBuilder("p")
        ...
        ;
      query.andWhere("forest_client.name like :forestClientName", { forestClientName:`%${forestClientName}%`})
      ...
      return result.map(project => {
        ...
        // TODO: Strip off timestamp, just return date...
        summary.commentingOpenDate = project.commenting_open_date;
        return summary;
      });
    } catch (error) {
