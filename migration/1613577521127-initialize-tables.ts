...
export class initializeTables1613577521127 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        // Create schema first
        // await queryRunner.createSchema("app_fom");
...
comment on column app_fom.interaction.revision_count is 'Standard column for optimistic locking on updates.';
    ...
    public async oldUp(queryRunner: QueryRunner): Promise<void> {
        ...
        // Create core tables - in reverse dependency order (parents first)
        ...
        Object.keys(foreignKeys).map(async (tableName) => {
            ...
            columns.map(async (col) => {
                ...
                let fkQuery = `ALTER TABLE app_fom.${tableName} ADD CONSTRAINT ${name}`;
                fkQuery += ` FOREIGN KEY (${col.columnNames})`;
                fkQuery += ` REFERENCES app_fom.${col.referencedTableName}(${col.referencedColumnNames.join(', ')})`;
                console.log(fkQuery);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        // Drop core tables in dependency order (children first)
